    ...
    <div class="container  flex justify-between items-center mx-auto px-8 md:px-14 lg:px-24 w-full">
        <section class="w-full">
            ...
            <div class="w-full grid lg:grid-cols-2 gap-8 lg:gap-32 mt-24">
                <div class="space-y-12">
                    <form action="{{ route('contact') }}" method="post">
                        ...
                        <div>
                            ...
                            @error('name')
                                <div class=" block mb-6 text-sm font-bold text-red-800">
                                    {{ $message }}
                                </div>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea type="text" name="message" class="w-full border border-input-border bg-input px-4 py-4 h-56 resize-none"></textarea>
                            @error('message')
                                <div class=" block mb-6 text-sm font-bold text-red-800">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="flex items-center justify-center">
                            <div class="datepicker relative form-floating mb-3 w-full" data-mdb-toggle-button="false">
                                <input type="text" name="date"
                                    class="form-control block w-full px-3 py-1.5 text-base font-normal  bg-input bg-clip-padding border border-solid  rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                                    placeholder="Select a date" data-mdb-toggle="datepicker" />
                                <label for="floatingInput" class="">Select a date to make meetings</label>
                ...
                <div class="mt-12">
                    <!-- Contact info -->
                    <p class="text-secondary">{{ $settings->phone_number }}</p>
                    <a href="mailto:email@mydomain.com" class="text-secondary underline mt-3 block">{{ $user->email }}</a>

                    <!-- Socials -->
                    <div class="flex mt-20 space-x-6">
                        <!-- Skype -->
                        <a href="{{ $settings->skype }}" target="_blank">
                            <svg class="w-8 h-8 lg:w-12 lg:h-12" width="50" height="50" viewBox="0 0 50 50"
                        ...
                        <a href="{{ $settings->twitter }}" target="_blank">
                            <svg class="w-8 h-8 lg:w-12 lg:h-12" width="42" height="42" viewBox="0 0 42 42"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
